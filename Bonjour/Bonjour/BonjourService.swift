...
extension BonjourService: NetServiceDelegate {
    ...
    private func netServiceDidPublish(_ sender: NetService) {
    ...
    private func netService(_ sender: NetService, didNotPublish errorDict: [String : NSNumber]) {
        if let netService = netService {
            delegates.values.forEach { (delegate) in
...
extension BonjourService: GCDAsyncSocketDelegate {
    ...
    private func socket(_ sock: GCDAsyncSocket!, didAcceptNewSocket newSocket: GCDAsyncSocket!) {
